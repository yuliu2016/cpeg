...
RULE(small_stmt) {
    ...
    (res = TOKEN(p, 64, "pass")) ||
    (res = TOKEN(p, 74, "break")) ||
    (res = TOKEN(p, 73, "continue")) ||
    (res = return_stmt(p)) ||
    (res = raise_stmt(p)) ||
    (res = del_stmt(p)) ||
    (res = nonlocal_stmt(p)) ||
...
RULE(atom) {
    ...
    (res = builder(p)) ||
    (res = TOKEN(p, 3, "NAME")) ||
    (res = TOKEN(p, 4, "NUMBER")) ||
    (res = TOKEN(p, 5, "STRING")) ||
    (res = TOKEN(p, 81, "None")) ||
    (res = TOKEN(p, 82, "True")) ||
    (res = TOKEN(p, 83, "False"));
    EXIT_FRAME(p);
}
